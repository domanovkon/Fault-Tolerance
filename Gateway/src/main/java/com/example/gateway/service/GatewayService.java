...
//    private static final String BUY_TICKETS = "http://192.168.50.240:8083/api/v1/tickets";
    ...
    private static BlockingQueue<String> ticketCancelQueue = new ArrayBlockingQueue<>(10);

    public ResponseEntity<Object> getBonusAccountPrivileges(String username) throws IOException, InterruptedException {
        ...
        if (response.statusCode() == 503) {
            BonusServiceUnavailable bonusServiceUnavailable = new BonusServiceUnavailable();
            bonusServiceUnavailable.setMessage("Bonus Service unavailable");
            return new ResponseEntity<Object>(bonusServiceUnavailable, HttpStatus.SERVICE_UNAVAILABLE);
        }
        ...
        return new ResponseEntity<Object>(bonusResponseDTO, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Object> buyTicket(String username, FlightBuyRequest flightBuyRequest) throws IOException, InterruptedException {
        ResponseEntity<Object> responseEntity = getBonusAccountPrivileges(username);
        ...
        BonusResponseDTO bonusResponseDTO = (BonusResponseDTO) getBonusAccountPrivileges(username).getBody();
        ...
        return new ResponseEntity<Object>(flightBuyResponseDTO, HttpStatus.OK);
    }
    ...
    public Object getUser(String username) throws IOException, InterruptedException {
        ...
        BonusResponseDTO bonusResponseDTO = new BonusResponseDTO();
        BuyTicketPrivilegeDTO buyTicketPrivilegeDTO = new BuyTicketPrivilegeDTO();
        ResponseEntity<Object> bonusResponse = getBonusAccountPrivileges(username);
        if (bonusResponse.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE) {
            userInformationDTO.setPrivilege(null);
            UserInfoBonusError userInfoBonusError = new UserInfoBonusError();
            userInfoBonusError.setTicketInformation(tickets);
            userInfoBonusError.setPrivileges(new ArrayList<>());
            return userInfoBonusError;
        }
        bonusResponseDTO = (BonusResponseDTO) bonusResponse.getBody();
    ...
    public ResponseEntity<Object> ticketCancel(UUID ticketUid, String username) throws IOException, InterruptedException {
        String uriStringGetTicket = UriComponentsBuilder.fromUriString(BUY_TICKETS + "/" + ticketUid).toUriString();
        HttpClient httpClient = HttpClient.newHttpClient();
        HttpRequest httpRequest = HttpRequest.newBuilder()
                .DELETE()
        ...
        try {
            ...
            ResponseEntity<Object> responseEntity = getBonusAccountPrivileges(username);
            if (responseEntity.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE) {
                ticketCancelQueue.add(ticketUid.toString() + ";" + username);
                TicketCancelConsumer ticketCancelConsumer = new TicketCancelConsumer();
                ticketCancelConsumer.start();
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            BonusResponseDTO bonusResponseDTO = (BonusResponseDTO) responseEntity.getBody();
    ...
    @Component
    public static class TicketCancelConsumer extends Thread {

        @Override
        public void run() {
            try {
                while (!GatewayService.ticketCancelQueue.isEmpty()) {
                    Thread.sleep(10000);
                    String query = ticketCancelQueue.take();
                    String username = query.split(";")[1];
                    UUID ticketUid = UUID.fromString(query.split(";")[0]);
                    ResponseEntity<Object> responseEntity = new GatewayService().getBonusAccountPrivileges(username);
                    if (responseEntity.getStatusCode() != HttpStatus.SERVICE_UNAVAILABLE) {
                        BonusResponseDTO bonusResponseDTO = (BonusResponseDTO) responseEntity.getBody();
                        List<BonusHistoryDTO> historyList = bonusResponseDTO.getHistory();
                        for (BonusHistoryDTO bh : historyList) {
                        ...
                        ticketCancelQueue.add(ticketUid + ";" + username);
                    }
                }
